...
export const sloarToLunar = function(sy: number, sm: number, sd: number): LunarInfo {
	...
	let ly: number | string = 0, lm: number | string = 0, ld: number | string = 0, lz: string = "";
	// 确定输出的农历年份
	for (let j = 0; j < calendar.lunarInfo.length; j++) {
		daySpan -= lunarYearDays(calendar.lunarInfo[j]);
		if (daySpan <= 0) {
			ly = 1949 + j;
			// 获取农历年份确定后的剩余天数
			daySpan += lunarYearDays(calendar.lunarInfo[j]);
	...
	// 确定输出的农历月份
	for (let k = 0; k < lunarYearMonths(calendar.lunarInfo[ly - 1949]).length; k++) {
		daySpan -= lunarYearMonths(calendar.lunarInfo[ly - 1949])[k];
		if (daySpan <= 0) {
			// 有闰月时，月份的数组长度会变成13，因此，当闰月月份小于等于k时，lm不需要加1
            const leapMonth = hasLeapMonth(calendar.lunarInfo[ly - 1949]) as number;
			if ( leapMonth && leapMonth <= k ) {
				if (leapMonth < k) {
					lm = k;
				} else if (hasLeapMonth(calendar.lunarInfo[ly - 1949]) === k) {
					lm = '闰' + k;
				} else {
					lm = k + 1;
				}
	...
	if (hasLeapMonth(calendar.lunarInfo[ly - 1949]) && (typeof(lm) === 'string' && lm.indexOf('闰') > -1)) {
        const num = /\d/.exec(lm);
		lm = `闰${lunarMonth[+(num || 1) - 1]}`
	} else {
		lm = lunarMonth[+(lm || 1) - 1];
	}
...
export const getYearMonthDay = function(sy: number, sm: number, sd: number): string {
	// 输入的月份减1处理
	sm -= 1;
	// 计算与公历基准的相差天数
	// Date.UTC()返回的是距离公历1970年1月1日的毫秒数,传入的月份需要减1
	let daySpan = (Date.UTC(sy, sm, sd) - Date.UTC(1949, 0, 29)) / (24 * 60 * 60 * 1000) + 1;
	let ly: number | string = 0, lm: number | string = 0, ld: number | string = 0;
	// 确定输出的农历年份
	for (let j = 0; j < calendar.lunarInfo.length; j++) {
		daySpan -= lunarYearDays(calendar.lunarInfo[j]);
		if (daySpan <= 0) {
	...
	for (let k = 0; k < lunarYearMonths(calendar.lunarInfo[ly - 1949]).length; k++) {
		...
		if (daySpan <= 0) {
			// 有闰月时，月份的数组长度会变成13，因此，当闰月月份小于等于k时，lm不需要加1
            const leapMonth1 = hasLeapMonth(calendar.lunarInfo[ly - 1949]) as number;
			if (leapMonth1 && leapMonth1 <= k) {
				if (leapMonth1 < k) {
					lm = k;
				} else if (leapMonth1 === k) {
					lm = '闰' + k;
				} else {
					lm = k + 1;
				}
...
/** @desc 计算农历年是否有闰月，参数为存储农历年的16进制; 农历年份信息用16进制存储，其中16进制的最后1位可以用于判断是否有闰月 **/
...
function lunarYearMonths(ly: number): number[] {
	...
	// 如果有闰月，需要把闰月的天数加上
    const leapMonth: number = hasLeapMonth(ly) as number;
	if (leapMonth) {
		monthArr.splice(leapMonth, 0, leapMonthDays(ly));
	}
...
function getTianGan(ly: number): string {
	...
	return calendar.Gan[tianGanKey - 1];
}

// 将农历年转换为地支，参数为农历年
function getDiZhi(ly: number): string {
	let diZhiKey: number = (ly - 3) % 12;
...
export const getYearMonthDayNew = (date: Date): DateInfo => {
	...
	return {
		...
		minute,
		second,
		weekNew: getWeek(week),
		time: formatDate(year, month, day)
	};
};
...
class Calendar {
	 ...
	 */
	leapMonth(y: number): number { //闰字编码 \u95f0
	 ...
	 */
	toChinaMonth(m: number): string | number { // 月 => \u6708
		if (m > 12 || m < 1) {
			return -1
		} //若参数错误 返回-1
	 ...
	 */
	solar2lunar(y: number, m: number, d: number): CalendarInfo | -1 { //参数区间1900.1.31~2100.12.31
		if (y < 1900 || y > 2100) {
			return -1;
		} //年份限定、上限
		if (y == 1900 && m == 1 && d < 31) {
		...
		return {
			...
			'monthCn': (isLeap ? "\u95f0" : '') + calendar.toChinaMonth(month),
			'dayCn': calendar.toChinaDay(day),
	 ...
	 */
	lunar2solar(y: number, m: number, d: number, isLeapMonth?: boolean | number): CalendarInfo | -1 { //参数区间1900.1.31~2100.12.1
	...
	$getDay(day: number, data: DateValue): string {
		...
		let gettimes: number = days.getTime() + 1000 * 60 * 60 * 24 * day;
		//setTime()以毫秒设置 Date 对象。
		days.setTime(gettimes);
		let year: number = days.getFullYear();
