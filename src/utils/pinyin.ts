...
// 查找字符对应的 PINYIN
function getPinyin(e: string) {
	for (var key in PINYIN) {
		const index = PINYIN[key as PinYinKeyType].indexOf(e);
		if (index != -1) {
			return formatPinyin(key)
		};
	}
	return false;
}
...
export class PinyinTranslate {
	...
	modeList: PinYinTranslateMode[] = [ 0, 1, 2, 10, 11, 12 ];
	...
	initInstance(
		...
		if(typeof options === "number" && this.modeList.includes(options)) {
			this.mode = options;
			this.options = {
				... this.options,
				... (ext || {})
			...
			const { mode } = options;
			if (this.modeList.includes(mode as PinYinTranslateMode)) this.mode = mode as PinYinTranslateMode;
			delete options.mode;
			this.options = {
				... this.options,
				... options,
		...
		else new Error("参数异常, [options]应为 Number 或 Object");
	}
	static createInstance(
        options: PinYinTranslateOptions | PinYinTranslateMode,
        ext?: PinYinTranslateOptions
    ) {
		if(!PinyinTranslate.instance) PinyinTranslate.instance = new PinyinTranslate(options, ext);
	...
	value(val: string): TranslateResult {
		...
		// 空格效验
		// 汉字效验
        const modeKey = "createMode" + this.mode;
	...
	translate(val: string = "", options?: PinYinTranslateExtOptions): TranslateResult {
		...
		for (let i = 0; i < len; i++) {
			...
			// 获取汉字的Pinyin
			let pinyin = getPinyin(char) as string;
			if(lowerCase && pinyin && i > 0){
				pinyin = pinyin.toLocaleLowerCase()
			}
			// 对于数字和字母不做处理
			const pre = i==0?"":split;
			if(!trimAll && !trim){
				value += pre + (pinyin || char);
				if(firstChar) pinyin = pinyin.slice(0,1);
				valueArr.push(pinyin || char);
			}
			else {
				if(reg.test(char)){
					if(!trimAll && trim){
					...
					if(pinyin){
						value += pre + pinyin;
						if(firstChar) pinyin = pinyin.slice(0,1);
						valueArr.push(pinyin);
					}
				}
			}

		}
		while (value.indexOf(split + split) > 0)
			value = value.replace(split + split, split);
		return {
			value,
			valueArr
		}
	...
	// 切割姓 与 名
	splitName = (e: string): { surname: string, name: string } => {
		let len = e.length,
			pre = "",
			suf = "",
            n = "",
		...
		if (len > 2) {
			let i = e.substr(0, 2);
			-1 !== SURNAME.indexOf(i) ? (n = i,
			r = e.substr(2)) : (n = e.substr(0, 1),
			r = e.substr(1))
		} else {
			2 == len ? (n = e.substr(0, 1), r = e.substr(1)) : n = e;
		}
		return {
			surname: n,
			name: r
